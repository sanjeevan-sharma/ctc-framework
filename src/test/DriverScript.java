...
public class DriverScript {
	...
	{
		// load Properties
		FileInputStream fis=new FileInputStream("D:\\workspace\\CTC_Testing\\src\\config\\config.properties");
		CONFIG=new Properties();
		CONFIG.load(fis);
		
		fis=new FileInputStream("D:\\workspace\\CTC_Testing\\src\\config\\OR.properties");
		OR=new Properties();
		OR.load(fis);
	...
	public void start() throws IllegalAccessException, IllegalArgumentException, InvocationTargetException{
		...
		suitexls=new Xls_Reader("D:\\workspace\\CTC_Testing\\src\\xls\\Suite.xlsx");
		...
		for(int suiterow=2;suiterow<=suitexls.getRowCount("Test Suite");suiterow++){
			if(suitexls.getCellData("Test Suite", "Runmode", suiterow).equals("Y")){
				...
				modulexls=new Xls_Reader("D:\\workspace\\CTC_Testing\\src\\xls\\"+suitexls.getCellData("Test Suite", "TSID", suiterow)+".xlsx");
